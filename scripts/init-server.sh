...
#
# CentOS 服务器初始化脚本
# 用于在腾讯云 CentOS 服务器上安装 Docker 和相关配置
#
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}   CentOS 服务器初始化脚本${NC}"
...
# 使用腾讯云镜像源
if get_tencent_mirror; then
  echo -e "${YELLOW}检测到腾讯云内网，使用腾讯云镜像加速器...${NC}"
  DOCKER_REPO_URL="https://mirrors.cloud.tencent.com/docker-ce/linux/centos/"
else
  echo -e "${YELLOW}使用 Docker 官方仓库...${NC}"
  DOCKER_REPO_URL="https://download.docker.com/linux/centos/"
fi

# 添加 Docker CE 仓库
if ! yum repolist | grep -q docker-ce; then
  yum-config-manager --add-repo ${DOCKER_REPO_URL}docker-ce.repo
fi
...
echo "   - STAGING_HOST: Staging 服务器 IP"
